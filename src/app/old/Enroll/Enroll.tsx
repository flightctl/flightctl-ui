...
const Enroll: React.FunctionComponent = () => {
  ...
  const fleets: ItemArrayType = ['none for now', 'default'];
  ...
  const buildApproval = async (enrollID: string) => {
    ...
    bodyApproval.labels['company.example.com/building'] =
      (document.getElementById('building') as HTMLInputElement)?.value ?? '';
    bodyApproval.labels['company.example.com/floor'] =
      (document.getElementById('floor') as HTMLInputElement)?.value ?? '';
    bodyApproval.labels['company.example.com/room'] =
      (document.getElementById('room') as HTMLInputElement)?.value ?? '';
    bodyApproval.labels['approval-comment'] =
      (document.getElementById('approvalComment') as HTMLInputElement)?.value ?? '';
    bodyApproval.region = (document.getElementById('region') as HTMLInputElement)?.value ?? '';
    console.log(JSON.stringify(bodyApproval));
    const approveEnrollmentResponse = await approveEnrollmentRequest(
  ...
  const buidRejection = async (enrollID: string) => {
    const rejectEnrollmentResponse = await rejectEnrollmentRequest(
      enrollmentrequest.metadata.name,
      auth?.user?.access_token ?? '',
    );
  ...
  return (
    <div>
      {approvalStatus ? (
        <EnrollStatus enrollID={enrollID} enrollStatus={approvalStatus} />
      ) : (
        <PageSection style={{ textAlign: 'center' }}>
          <Title headingLevel="h1" size="lg" style={{ marginBottom: '20px' }}>
            Enrollment Request
          </Title>
          <div>
            <ClipboardCopy
          ...
          <div style={{ marginBottom: '20px', marginTop: '20px' }}>
            <Button
              variant="primary"
              style={{ fontSize: '30px', padding: '20px 20px' }}
              onClick={() => buildApproval(enrollmentrequest.metadata.name)}
            >
              Approve
            </Button>
          </div>
              ...
              <Alert
                ...
                actionLinks={
                  <React.Fragment>
                    <AlertActionLink
                      onClick={() => {
                        buidRejection(enrollmentrequest.metadata.name);
                        handleCancel();
                      }}
                    >
                      Reject enrollment
