    ...
    <PageSection>
      ...
      >
                  ...
                  <tbody>
                    <tr>
                      <td>
                        <p>
                          Enroll date:{' '}
                          {(device.contents as any)?.metadata?.creationTimestamp
                            ? dateFormatter((device.contents as any).metadata.creationTimestamp)
                            : ''}
                          <br />
                          Next sync: 0h 07m 20s
                          <br />
                          Last contact:{' '}
                          {device.contents &&
                          (device.contents as any).metadata &&
                          (device.contents as any).metadata.creationTimestamp
                            ? durationLastContact((device.contents as any).metadata.creationTimestamp)
                            : ''}
                          <br />
                          Last sync status: OK
                          <br />
        ...
        <Tab
          ...
          aria-label="Default content - Configuration"
        >
          <table style={{ width: '100%' }}>
            <tbody>
              <tr>
                  ...
                  <Panel style={{ padding: '10px', verticalAlign: 'top' }}>
                    ...
                    <PanelMain>
                      <PanelMainBody>
                                ...
                                <Dropdown
                                  ...
                                  toggle={(toggleRef: React.Ref<MenuToggleElement>) => (
                                    <MenuToggle
                                      ref={toggleRef}
                                      isFullWidth
                                      onClick={onFleetToggleClick}
                                      isExpanded={isOpenFleet}
                                    >
                                  ...
                                  <TextInput
                                    ...
                                    onChange={(_event, key) =>
                                      setTableRows((prevRows) =>
                                        prevRows.map((row, i) => (i === index ? { ...row, key } : row)),
                                      )
                                    }
                                  />
                                </td>
                                <td style={cellStyle}>
                                  <TextInput
                                    ...
                                    onChange={(_event, value) =>
                                      setTableRows((prevRows) =>
                                        prevRows.map((row, i) => (i === index ? { ...row, value } : row)),
                                      )
                                    }
                                  />
                                </td>
                        ...
                        <Button style={{ margin: '10px' }} variant="tertiary" onClick={addNewLabel}>
                          Add New Label
                        </Button>

                        <Button variant="primary" onClick={saveLabels}>
                          Save changes
                        </Button>
                      </PanelMainBody>
                    </PanelMain>
                ...
                <td style={{ padding: '4px 0px 0px', width: '65%', verticalAlign: 'top' }}>
                  <div>
                    <CodeBlock>
                      <CodeBlockCode
                        style={{ background: '#212427', color: '#ffffff', padding: '10px' }}
                        id="code-content"
                      >
                        {YAML.stringify(device.contents)}
                      </CodeBlockCode>
                    </CodeBlock>
                  </div>
                </td>
        ...
        <Tab eventKey={2} title={<TabTitleText>Actions</TabTitleText>}>
          <Card style={{ width: '50%' }} ouiaId="remoteControlCard">
            ...
            <CardBody>
              Enable remote control on the device, with this enabled flightctl-agent on device will run a temporary
              component to enable remote access for troubleshooting.
            </CardBody>
            <CardBody>
              <span style={{ width: '50px' }}>
                TTL: (minutes) <TextInput type="number" id="ttl" name="ttl" aria-label="TTL" />
              </span>
              <br />
              <Checkbox
                label="Enable remote Terminal"
                defaultChecked
                isDisabled
                id="terminal"
                name="terminal"
                aria-label="Enable remote Terminal"
              />
              <br />
              <Checkbox
                label="Enable OpenTelemetry-collector"
                id="otel"
                name="otel"
                aria-label="Enable OpenTelemetry-collector"
              />
              <TextArea
                id="otelconfig"
                name="otelconfig"
                aria-label="OpenTelemetry-collector config"
                isDisabled={true}
        ...
        <Tab eventKey={3} title={<TabTitleText>Terminal</TabTitleText>}>
          <div>{activeTabKey === 3 && <RemoteControl />}</div>
        </Tab>
      </Tabs>
    </PageSection>
  );
