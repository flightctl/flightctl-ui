...
func (o *OIDCAuthHandler) GetUserInfo(token string) (string, error) {
	resp, statusCode := getUserInfo(token, o.tlsConfig, o.authURL, o.userInfoEndpoint)
	...
	if err := json.Unmarshal(resp, &userInfo); err != nil {
		return "", fmt.Errorf("failed to marshal OIDC user response: %w", err)
	}
