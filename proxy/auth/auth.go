...
// Gets the API token for a given sessionId. Deletes the mapping immediate after retrieval
...
func (a *AuthHandler) CreateSessionToken(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		// Store the token, mapping it to a new sessionId that can be used to access the token a single time.
		...
		// Return sessionId and keep the token stored internally
...
func (a *AuthHandler) GetSessionToken(w http.ResponseWriter, r *http.Request) {
	...
	// Retrieve the token associated to the sessionId. Deletes the mapping so the token cannot be retrieved again for the same sessionId
