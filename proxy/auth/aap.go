...
func (a *AAPAuthHandler) GetUserInfo(token string) (string, error) {
	userInfoEndpoint := fmt.Sprintf("%s/api/gateway/v1/me/", a.internalAuthURL)
	resp, statusCode := getUserInfo(token, a.tlsConfig, a.authURL, userInfoEndpoint)
	...
	return userInfo.Results[0].Username, nil
}
