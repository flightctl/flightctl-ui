...
import DynamicForm, { CatalogVolumSelection } from '../../../DynamicForm/DynamicForm';
...
export const DynamicAppForm = ({ schemaErrors, isEdit }: DynamicAppFormProps) => {
  ...
  const formContext = React.useMemo(() => {
    const onVolumeSelected = (selection: CatalogVolumSelection) => {
      const existing = values.volumeSelection.findIndex((a) => a.volumeIndex === selection.volumeIndex);
      let newVolumeSelection: CatalogVolumSelection[];
      if (existing >= 0) {
